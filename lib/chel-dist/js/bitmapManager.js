...
{ 
	...
	BitmapManager.prototype.setImagesPath = function (_path)
	{
		this.m_imagesPath = _path;
	};
	...
	BitmapManager.prototype.performAsynchLoadBase = function(arr, _callback, _owner)
	{
		...
		else
		{
			...
			for (var i=0; i < arr.length; i++)
			{
				...
				newItem.m_originalBitmap.src =  BitmapManager.self.m_imagesPath + arr[i];
				...
				newItem.m_originalBitmap.onload = function(_event)
				{
					...
					msglog('BitmapManager resource loaded:' + BitmapManager.self.m_imagesPath + this.src + ", itemIndex=" + itemIndex);
				...
				{
					msglog('ERROR: BitmapManager resource loaded:' + BitmapManager.self.m_imagesPath + this.src);
					_owner.m_managerAvailable = false;
					updateProgressBar();
				}
			}
